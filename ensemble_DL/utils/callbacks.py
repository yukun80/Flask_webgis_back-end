...
class LossHistory:
    def __init__(self, log_dir, model, input_shape, val_loss_flag=True):
        ...
        try:
            # 修改通道数
            # 测试模型是否可以被正确的画出来
            dummy_input = torch.randn(2, 9, input_shape[0], input_shape[1])
...
class EvalCallback:
    def __init__(
        ...
        self.image_ids = [image_id.split()[0] for image_id in image_ids if image_id.lower().endswith(".tif")]
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            self.net = model_eval
            gt_dir = os.path.join(self.dataset_path, "ann_dir", "val")
            ...
            for image_id in tqdm(self.image_ids):
                ...
                # 读取验证图像
                image_path = os.path.join(self.dataset_path, "img_dir", "val", image_id + ".tif")
                ...
                new_array = np.rollaxis(gdal_array, 0, 3)
                image = new_array  # (C,H,W) --> (H,W,C)
                # ------------------------------#
                #   获得预测txt
                # ------------------------------#
                # image = self.get_miou_png(image)
                image = self.get_miou_tif(image)
                # image.save(os.path.join(pred_dir, image_id + ".png"))
                # 将结果保存为单通道tif图像
                output_path = os.path.join(self.miou_out_path, "detection-results", f"{image_id}.tif")
                driver = gdal.GetDriverByName("GTiff")
                outdata = driver.Create(output_path, width, height, 1, gdal.GDT_Byte)
            ...
            # 将 miou 写入 TensorBoard
            self.writer.add_scalar("miou", temp_miou, epoch)
